...
{
  template <class T>
  inline const unsigned char * ucast(const T * ptr)
  {
...
// find the start or end of each bucket
    void getBuckets(const unsigned char *s, int *bkt, int n, int K, int cs, bool end) {
      int i, sum=0;
...
// compute SAl
    void induceSAl(const unsigned char *t, int *SA, const unsigned char *s, int *bkt,
...
// compute SAs
    void induceSAs(const unsigned char *t, int *SA, const unsigned char *s, int *bkt,
                   int n, int K, int cs, bool end) {
      int i, j;
...
// use a working space (excluding s and SA) of at most 2.25n+O(1) for a constant alphabet
    void SA_IS(const unsigned char *s, int *SA, int n, int K, int cs) {
      int i, j;
...
// recurse if names are not yet unique
      ...
      if(name<n1)
        SA_IS(ucast(s1), SA1, n1, name-1, sizeof(int));
